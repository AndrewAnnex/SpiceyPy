...
from spiceypy.tests.gettestkernels import (
    get_standard_kernels, write_test_meta_kernel, cleanup_core_kernels,
    CoreKernels
)
import pytest
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            1, len(CoreKernels.standardKernelList) + 1, 
            [CoreKernels.lsk], CoreKernels.testMetaKernel
        ),
        (
            len(CoreKernels.standardKernelList) + 1, 
            len(CoreKernels.standardKernelList) + 1,
            CoreKernels.testMetaKernel, CoreKernels.testMetaKernel
        ),
        (
            len(CoreKernels.standardKernelList) + 1, 1,
            CoreKernels.testMetaKernel, [CoreKernels.lsk]
        )
